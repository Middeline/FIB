...
writeClauses(K):-
	oneTaskMaxPerHourAndGangster, 	% Un gangster en una hora donada com a molt pot fer una tasca
	requiredPerHour,					% Fija los gangsters necesarios haciendo cierta tarea en cierta hora
	maxConsecutiveHours(K), 			% Cap gangster treballa més de K hores consecutives
...
oneTaskMaxPerHourAndGangster:-
	gangster(G), hour(H),
	findall(does(G, T, H), task(T), Lits), atMost(1, Lits), fail.
oneTaskMaxPerHourAndGangster.



requiredPerHour:-
	hour(H), needed(T, H, N), findall(does(G, T, H), gangster(G), Lits), exactly(N, Lits), fail.
requiredPerHour.



gangsterNotAvailable:-
	gangster(G), blocked(G, H), findall(does(G, T, H), (task(T), hour(H)), Lits), atMost(0, Lits), fail.
gangsterNotAvailable.



maxConsecutiveHours(K):-
	task(T),
	gangster(G),
	HMAX is 72 - K,
	between(1,HMAX,H),
	HK is H+K,   % En of block
	findall(does(G, T, HBETWEEN), between(H, HK, HBETWEEN), Lits),
	negateAll(Lits, NLits),
	writeClause(NLits),
	fail.
maxConsecutiveHours(_).

eachGangsterRestHourBeetweenJobs:-
  gangster(G),
  between(1,71,H), H1 is H + 1,
  task(T1), task(T2), T1 \= T2,
  writeClause([-does(G, T1, H), -does(G, T2, H1)]),
  fail.
eachGangsterRestHourBeetweenJobs.
...
% El cost es el máxim numero d'hores treballades consecutivament per algun gangster
costOfThisSolution(M,K):-
	between(0, 72, N), K is 72-N, gangster(G), gangsterTeballaConsecutivament(G, K, M).

gangsterTeballaConsecutivament(G, K, M):- llistaHoresTreballades(G, M, L), hiHaSubllistaHoresConsecutives(K, L).

llistaHoresTreballades(G, M, L) :- numlist(1, 72, NUMS), include(esHoraTreballada(G, M), NUMS, L).

esHoraTreballada(G, M, H):- member( does(G, _, H), M).

hiHaSubllistaHoresConsecutives(K, L):- append([_, LK, _], L), length(LK, K), horesConsecutives(LK). 

horesConsecutives([]).
horesConsecutives([_]).
horesConsecutives([X,Y|L]) :- Y = X1, X1 is X+1, horesConsecutives([Y|L]).
