...
writeClauses(K) :-
    cadahoraunatask,    %gangster no pot fer 2 coses alhora
    maximhores(K),         % cap gangster treballa més de K hores consecutives
    necesitado,
    gangsterocupado,       % gangster esta ocupado en una hora en concreto
    descanso,              % descanso entre tareas distintas
    true, !.
    ...
    descanso :-
        ...
        task(T1), task(T2), T1 \= T2,  %distintas tareas
        writeClause([ -does(G, T1, H), -does(G, T2, H1)]), fail. %no seguidas
    descanso.
    ...
    maximhores(K) :-
        task(T), gangster(G), hour(H), H1 is H + K, hour(H1),
        findall( does(G, T, H2), between(H, H1, H2), Lits),
        negateAll( Lits, NewLits), writeClauses(NewLits), fail.        
