...
satVariable( hasDouble(T) ) :- team(T), !.  % meaning that team T gets a double.
...
writeClauses:-
    ...
    sixOrSevenHomes,           
    atMostOneDouble, 
    juguen_a_casa,          %impose all the "home" constraints given below.
    tenen_double,           %per a "inicialitzar sat variable"
    max_12_tenen_double,    %per a imposar que maxim 12 equips poden tenir double
    true.
...
tenen_double :-
    team(S), round(R),
    writeClause([ -double(S,R), hasDouble(S) ]),
    fail.
tenen_double.

max_12_tenen_double :- 
    findall( hasDouble(T), team(T), Lits),
    atMost(12, Lits),
    fail.
max_12_tenen_double.
